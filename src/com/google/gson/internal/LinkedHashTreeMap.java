...
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
	@SuppressWarnings({ "unchecked", "rawtypes" }) // to avoid
													 // Comparable<Comparable<Comparable<...>>>
	private static final Comparator<Comparable> NATURAL_ORDER = (a, b) -> a.compareTo(b);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" }) // unsafe! if comparator is
	...
	void removeInternal(Node<K, V> node, boolean unlink) {
		...
		if (left != null && right != null) {
			...
			removeInternal(adjacent, false); // takes care of rebalance and
											 // size--
	...
	/**
	 * Returns a new array containing the same nodes as {@code oldTable}, but
	 * with twice as many trees, each of (approximately) half the previous size.
	 */
	 ...
	 * left, right and parent links are <strong>no longer used</strong>. For
	 * this reason it is safe to transform these links as you walk a tree.
	 *
	 * <p>
	 * <strong>Warning:</strong> this iterator is destructive. It clears the
	 ...
	 * <i>N</i>, the height of <i>N.left</i> and <i>N.right</i> is different by
	 * at most 1. It accomplishes this by omitting deepest-level leaf nodes when
	...
	final static class AvlBuilder<K, V> {
		...
		void reset(int targetSize) {
			// compute the target tree size. This is a power of 2 minus one,
			// like 15 or 31.
		...
		void add(Node<K, V> node) {
			...
			/*
			 * Combine 3 nodes into subtrees whenever the size is one less than
			 * a multiple of 4. For example we combine the nodes A, B, C into a
			 * 3-element tree with B as the root.
			 *
			 * Combine two subtrees and a spare single value whenever the size
			 * is one less than a multiple of 8. For example at 8 we may combine
			 * subtrees (A B C) and (E F G) with D as the root to form ((A B C)
			 * D (E F G)).
			 *
			 * Just as we combine single nodes when size nears a multiple of 4,
			 * and 3-element trees when size nears a multiple of 8, we combine
			 * subtrees of size (N-1) whenever the total size is 2N-1 whenever N
			 * is a power of 2.
			 */
			for (int scale = 4; (size & scale - 1) == scale - 1; scale *= 2) {
				if (leavesSkipped == 0) {
					// Pop right, center and left, then make center the top of
					// the stack.
					...
					// Pop right and center, then make center the top of the
					// stack.
					Node<K, V> right = stack;
					Node<K, V> center = right.parent;
					stack = center;
