...
public class ISO8601Utils {
	...
	/**
	 * Format a date into 'yyyy-MM-ddThh:mm:ssZ' (default timezone, no
			...
			if (hasT) {
				...
				if (date.length() > offset) {
					...
					if (c != 'Z' && c != '+' && c != '-') {
						...
						if (checkOffset(date, offset, '.')) {
							offset += 1;
							int endOffset = indexOfNonDigit(date, offset + 1); // assume
																				 // at
																				 // least
																				 // one
																				 // digit
							int parseEndOffset = Math.min(endOffset, offset + 3); // parse
																					 // up
																					 // to
																					 // 3
																					 // digits
							...
							switch (parseEndOffset - offset) { // number of
																 // digits
																 // parsed
								case 2:
	...
	/**
	 * Returns the index of the first character in the string that is not a
	 * digit, starting at offset.
	 */
