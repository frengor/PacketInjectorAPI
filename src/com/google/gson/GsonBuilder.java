 ...
 *
 * <pre>
 * Gson gson = new GsonBuilder().registerTypeAdapter(Id.class, new IdTypeAdapter()).enableComplexMapKeySerialization()
 * 		.serializeNulls().setDateFormat(DateFormat.LONG).setFieldNamingPolicy(FieldNamingPolicy.UPPER_CAMEL_CASE)
...
public final class GsonBuilder {
	 ...
	 * various configuration settings. GsonBuilder follows the builder pattern,
	 * and it is typically used by first invoking various configuration methods
	 * to set desired options, and finally calling {@link #create()}.
	 */
	public GsonBuilder() {
	}
	 ...
	 *
	 * @param modifiers
	 *            the field modifiers. You must use the modifiers specified in
	 *            the {@link java.lang.reflect.Modifier} class. For example,
	 ...
	 * generated JSON with some special text. This prevents attacks from
	 * third-party sites through script sourcing. See
	 * <a href="http://code.google.com/p/google-gson/issues/detail?id=42">Gson
	 * Issue 42</a> for details.
	 *
	 * @return a reference to this {@code GsonBuilder} object to fulfill the
	 *         "Builder" pattern
	 * @since 1.3
	 ...
	 * serialization or deserialization that do not have the
	...
	/**
	 * Configure Gson to serialize null fields. By default, Gson omits all
	 * fields that are null during serialization.
	 *
	 * @return a reference to this {@code GsonBuilder} object to fulfill the
	 *         "Builder" pattern
	 * @since 1.2
	 ...
	 * is a complex type (i.e. non-primitive) in its <strong>serialized</strong>
	 * JSON form. The default implementation of map serialization uses
	 * {@code toString()} on the key; however, when this is called then one of
	 * the following cases apply:
	 *
	 * <h3>Maps as JSON objects</h3> For this case, assume that a type adapter
	 * is registered to serialize and deserialize some {@code Point} class,
	 * which contains an x and y coordinate, to/from the JSON Primitive string
	 ...
	 * {
	 * 	&#64;code
	 * 	Gson gson = new GsonBuilder().register(Point.class, new MyPointTypeAdapter()).enableComplexMapKeySerialization()
	 * 			.create();
	 *
	 ...
	 * </pre>
	 *
	 * <h3>Maps as JSON arrays</h3> For this case, assume that a type adapter
	 * was NOT registered for some {@code Point} class, but rather the default
	 * Gson serialization is applied. In this case, some {@code new Point(2,3)}
	 * would serialize as {@code {"x":2,"y":5}}.
	 *
	 * <p>
	 * Given the assumption above, a {@code Map<Point, String>} will be
	...
	/**
	 * Configures Gson to apply a specific serialization policy for {@code Long}
	 * and {@code long} objects.
	 *
	 ...
	 * {@code strategies} suggests that a field (or class) should be skipped
	 * then that field (or object) is skipped during
	 ...
	 * exclusion strategy objects then the exclusion strategies that were added
	 * will be applied as a disjunction rule. This means that if one of the
	 * added exclusion strategies suggests that a field (or class) should be
	 * skipped then that field (or object) is skipped during its serialization.
	 *
	 * @param strategy
	 ...
	 * will be applied as a disjunction rule. This means that if one of the
	 * added exclusion strategies suggests that a field (or class) should be
	 * skipped then that field (or object) is skipped during its
	 * deserialization.
	 *
	 * @param strategy
	...
	/**
	 * Configures Gson to serialize {@code Date} objects according to the
	 * pattern provided. You can call this method or {@link #setDateFormat(int)}
	...
	/**
	 * Configures Gson to to serialize {@code Date} objects according to the
	 * style value provided. You can call this method or
	 * {@link #setDateFormat(String)} multiple times, but only the last
	 * invocation will be used to decide the serialization format.
	 *
	 * <p>
	 * Note that this style value should be one of the predefined constants in
	 * the {@code DateFormat} class. See the documentation in
	 ...
	 * combines the registration of an {@link TypeAdapter},
	 * {@link InstanceCreator}, {@link JsonSerializer}, and a
	 * {@link JsonDeserializer}. It is best used when a single object
	 * {@code typeAdapter} implements all the required interfaces for custom
	 * serialization with Gson. If a type adapter was previously registered for
	 * the specified {@code type}, it is overwritten.
	 *
	 * <p>
	 ...
	 * {@link TypeAdapter}, {@link JsonSerializer} and a
	 * {@link JsonDeserializer}. If a type adapter was previously registered for
	 ...
	 *
	 * @param baseType
	 *            the class definition for the type adapter being registered for
	 *            the base class or interface
	 * @param typeAdapter
	 *            This object must implement at least one of
	 ...
	 * specification</a> disallows special double values (NaN, Infinity,
	 * -Infinity). However, <a href=
	 * "http://www.ecma-international.org/publications/files/ECMA-ST/Ecma-262.pdf">Javascript
	 * specification</a> (see section 4.3.20, 4.3.22, 4.3.23) allows these
	 ...
	 * level, it makes sense to accept these values as valid JSON even though
	 * JSON specification disallows them.
	 *
	 * <p>
	 ...
	 * method is free of side-effects to this {@code GsonBuilder} instance and
	 * hence can be called multiple times.
	 *
	 * @return an instance of Gson configured with the options currently set in
	 *         this builder
	 */
	public Gson create() {
		...
		return new Gson(excluder, fieldNamingPolicy, instanceCreators, serializeNulls, complexMapKeySerialization,
				generateNonExecutableJson, escapeHtmlChars, prettyPrinting, serializeSpecialFloatingPointValues,
